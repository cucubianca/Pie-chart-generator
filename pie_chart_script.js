document.addEventListener("DOMContentLoaded", () => {
    ...
    formatButtons.forEach(btn => btn.classList.remove("active", "pressed"));//all format buttons start unselected

    function drawChart() {
        const rows = document.querySelectorAll(".data-row");//collects all data rows 
        let data = [];
        let total = 0;
        ...
        rows.forEach(row => {
            ...
            if (label && value > 0) { //validates the data
        ...
        ctx.clearRect(0, 0, canvas.width, canvas.height); //wipes the chart
        ...
        //draws each slice, starting at the top 12 oclock
        ...
        ctx.restore(); //ends the clipping region
        ...
        legend.innerHTML = ""; //clears old legend items
        data.forEach(item => {
            const percent = ((item.value / total) * 100).toFixed(1);
            const legendItem = document.createElement("div");
            legendItem.classList.add("legend-item"); //adds a color box, label and percentage
    ...
    generateBtn.addEventListener("click", () => { 
        ...
        overlay.style.display = "flex"; //shows animation 
        setTimeout(() => { overlay.style.display = "none"; //hides animation 
        drawChart(); 
        }, 1500); //1.5 seconds 
    });
    ...
    defaults.forEach(item => {
        ...
        row.classList.add("data-row");//new data row
        row.innerHTML = ` 
    ...
    downloadBtn.addEventListener("click", () => {
        ...
        //draw pie chart for the export canvas
